...
export default defineConfig({
    ...
    build: {
        outDir: join(__dirname, 'dist/dashboard'), // [!code highlight]
    },
    plugins: [
        vendureDashboardPlugin({
            vendureConfigPath: pathToFileURL('./src/vendure-config.ts'),
            // When served by DashboardPlugin in production, the dashboard can
            // derive the API URL from the same origin that served the app.
    ...
    plugins: [
        ...
        DashboardPlugin.init({
            ...
            appDir: IS_DEV
                ...
                : path.join(__dirname, 'dashboard'), // [!code highlight]
        }),
    ],
};
```
...
2. **Deploy the contents of the `dist/dashboard` directory to your hosting service**
