...
本文用于当前 Vendure 全球独立站的上线准备。前台提供简体中文和英文，店铺 Channel 用于区分店铺和默认币种，不用来按国家切分公开站点。
...
- `模拟支付（测试）` 由后台“店铺综合设置 → 支付与交付”开关控制；关闭时 Shop API 不会把它返回给结账页。
- 开启后不会产生真实扣款，但付款记录会直接进入 `Settled`，订单按付款成功继续后续流程。
...
- 确认经营主体、默认币种、商品价是否含税、支付方式和退款规则。
- 根据实际销售国家和地区确认跨境数字商品税务，必要时为不同司法辖区建立独立税区。
- 明确数字商品与实物商品各自的交付方式。
- 删除或禁用 `crud-shipping`、`测试支付` 等测试配置。
- 使用不同国家或地区的真实地址完成下单、税费和支付测试。
...
验收标准：正式域名访问到正确店铺；邮件中的链接回到该独立站，不串 Channel、不使用本地地址。
...
- 当前在售商品不得存在 `DEMO-` SKU，也不得引用带有 `storefront-demo` 标签的资源；软删除商品和历史订单引用的资源应保留。
- 替换首页及分类页临时内容，并逐项确认链接目标。
- 分别使用简体中文和英文完成桌面端与移动端全流程验收。
- 覆盖加载、空状态、接口错误、图片失败、无库存、支付失败和重复提交。
- 核对隐私政策、服务条款、退款政策、配送政策及经营主体信息。

验收标准：使用不同国家或地区的真实数据完成测试订单、支付、履约、退款和邮件通知，且金额与语言正确。
...
READINESS_TAX_POLICY_JSON='{"my-malaysia":{"pricesIncludeTax":false,"rates":{"Standard Tax":0}}}' \
bun run --cwd packages/dev-server audit:storefront-readiness
```

`READINESS_TAX_POLICY_JSON` 必须来自已批准的业务税价规则，上述布尔值和 `0` 税率仅为命令格式示例。未提供时预检会保留人工阻塞，不会猜测价格是否含税或税率。

预检会拒绝未验证域名、错误税区/配送区、测试支付或配送、当前在售的 `DEMO-` 商品或演示资源、演示装修内容、缺失中英文、缺图/缺价以及搜索索引积压。软删除商品和历史订单保留的资源不会阻断发布。命令为只读数据检查，除了管理员登录会话外不会写入业务数据；失败时返回非零退出码，可作为发布流水线阻断条件。
...
实际进程由 `deploy/switch-production-runtime.sh` 使用 `deploy/ecosystem.production.config.cjs` 重建 PM2 进程定义，必须传入已验证候选目录的绝对路径。不能用 PM2 reload 切换不可变运行目录，因为它会保留旧的 `pm_cwd` 和 `pm_exec_path`。生产 EC2 不执行 `bun install`、Vendure CLI 或前端构建；`.env`、上传资产和数字交付文件不进入产物。完整命令、校验和回滚步骤见 `deploy/DEPLOYMENT_RUNBOOK.md`。
...
- [ ] 当前在售的 `DEMO-` 商品与演示资源已被真实内容替换，软删除商品和历史订单资源已保留。
