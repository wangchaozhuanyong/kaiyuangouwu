...
`Production Release` 运行编号和同一 SHA 的手动 `Monitor Production Health` 验收运行编号。
...
- 每次运行时发布在下载候选前，会在同一部署锁内核验 `current` 指针、`current-sha` 和两个 PM2
  进程均指向同一在线运行产物，然后只清理排序晚于当前运行产物的失败候选目录及对应 `.tar.gz`，并先执行磁盘门禁。
  该自动清理不会删除当前产物、任何更早的回滚产物、数据库备份或日志；旧回滚产物仍只由上述成功发布后的保留策略处理。
  磁盘门禁还须在暂停 Worker 前保留两份当前运行产物的暂存余量及根盘健康阈值余量；不足时应核对保留计划或扩容，不得直接提高健康阈值。
...
普通前端使用相应包的关联 Vitest 测试与 `bun run build`；测试使用 `NODE_ENV=test`，制品使用 `NODE_ENV=production`。后端使用 `scripts/ci-run.mjs` 按计划调用 Lerna，并在测试前恢复已构建的必要输出。公共输入变化可以影响多个包，但不会把 `full` 自动设为真。全量检查只在明确要求时运行。生产环境、运行产物及 High+ 审计门禁仍然执行；不得使用开发工作区遗留产物替代验证。
...
一次发布保留一份检查账本；通过后，输入未变且没有新失败，不再重复本地/CI 测试。部署后的备份验收读取同次恢复演练结果，不再次恢复全库，也不为重复核验下载整个制品。排队无进展时报告瓶颈，不靠重复调度解决。

入口依次执行现场预检、累计范围判断、查找可复用检查、补齐缺失检查、构建或恢复产物、部署和相关验收。检查证据按实际输入复用：每个包及其依赖源码、锁文件、执行命令、运行环境保持一致，且来源为本仓库可信成功运行时，可跨源码树组合复用；前端关联测试还必须覆盖全部待发布文件。目标版本须有成功 CI 作为锚点，旧证据不能独自批准新版本。`release-evidence.mjs plan` 输出每项输入摘要、证据运行号和缺失项；工作流仅执行缺失项。v2 证据只记录本轮实际执行的检查，最终门禁拒绝计划内任务被跳过。后续部署失败时，只有经远端确认 all-passed 成功的 CI 阶段证据可继续复用；失败的 CI 门禁不能提供通过证明。完整前端证据匹配实际执行的根构建/测试任务，关联测试证据匹配前端任务。Next Admin 独立构建先补 core/common 类型声明，不运行后端测试。证据缺失、过期或不匹配时补对应检查，不自动重跑全仓。前端和运行时编译产物另外验证 SHA-256、源码树、Node `24.19.0`、Bun `1.3.14`、Linux x64 和影响构建的环境指纹。前端制品有效时连依赖安装也跳过。缺失产物只补构建；运行时只能复用完整生产构建，部分插件输出不能充当完整运行时。
...
登录/注册页文案、色板和标签属于 Vendure 内容，不得只改客户端。在 `Production Release` 中勾选 `auth_visuals`，并填写已审核 Channel；发布链会先 dry-run，再以带 `expectedUpdatedAt` 的单个 Admin API 批次原子写入，随后运行独立只读 `--verify` 反查 Admin 与中英文 Shop API，并记录 `AUTH_VISUAL_VERIFY_OK`。验证失败时恢复原内容并停止切换。
...
`Production Release` 唯一入口填写完整 manifest key/勾选 `auth_visuals`/勾选 `moyao_brand`，并填写已审核 Channel code；大马通整店内容则勾选 `damatong_storefront` 并填写 `damatong_channel_token=my-malaysia`。不再另行触发第二个内容发布工作流，也不回退到服务器默认 Channel。
...
若目标提交包含 MOYAO AI 品牌更新，必须在该 SHA 的 `Production Release` 中设置 `moyao_brand=true` 与 `channel_codes=__default_channel__`。下游将它与制品 SHA-256 作为同一份发布计划校验，并在唯一生产锁内先预演，再把三套官方品牌素材、双语品牌名/口号和色板一次性绑定到主 Channel：
...
品牌颜色在发布计划、Admin/Shop 读回校验和恢复原绑定时统一为大写十六进制，匹配后台持久化格式；仅大小写不同不算内容变化，实际颜色差异仍必须阻断校验。已绑定素材的区块与条目按 Asset ID 比较图片，后台从素材生成的 imageUrl 不算额外变更；没有 Asset ID 的旧图片仍逐项校验原 URL。
...
2. 在“店铺综合设置 → 编辑店铺档案 → 品牌图片”分别选择方形店铺图标、浅色背景 Logo 和深色背景 Logo。需要复用默认素材库中已有的图片时，明确选择“默认店铺素材”；保存会先将所选素材加入目标店铺，再保存该店铺品牌关联，不移动或删除原素材。
3. 保存使用目标档案的渠道及 `expectedUpdatedAt`，只提交改过的品牌关联。分配失败或档案版本冲突时停止，刷新并核对后重试；不要重复上传图片，也不要更改其他店铺以绕过错误。
4. 回读后台档案，并按客户端实际语言路由分别检查中英文 Shop API 的三个素材 ID、地址与名称；最后刷新目标域名前台，检查“左侧图标、右侧店铺名称”。另查默认店铺品牌关联保持原值。三方一致才算发布完成。
5. 保存前后版本、素材 ID、原文件 SHA-256 和验收截图写入本次发布记录。若核对失败，通过同一后台恢复原品牌关联并重新核对，保留新旧素材以便追溯。
...
发布期间冻结新的生产合并：发现已有 `Production Release` 正在运行时，后续 PR 保持 ready 状态但不合并。普通代码发布目标为 30–45 分钟；包含审核数据或数据库迁移时目标为 45–90 分钟。超过 90 分钟立即停止启动新发布，按预检、构建、上传、部署、验收标记具体阻塞阶段，不以重复调度掩盖问题。已对同一 SHA 和相同输入成功的门禁不得无故重复；报告、分支清理和非阻塞工作树整理放在生产切换与验收之后。
...
12. 完成发布记录中的制品名称、制品 SHA-256、`Production Release` 工作流编号、UTC 时间和验收结果；兼容旧记录字段时，制品与部署工作流编号填写同一个 `Production Release` 运行编号。记录必须能够唯一定位生产运行的代码和制品。
...
验收脚本会确认主域名账户入口和公开品牌查询可访问，匿名商品查询必须返回 `FORBIDDEN`；即使携带推广入口 Cookie，也不能读取商品。`/promo` 推广页与签名进入按钮继续可用，站点地图只公开推广页。发布时须在各店铺 CDN 绕过商品接口与媒体路径的共享缓存，并清理历史缓存；复验旧商品图片直链拒绝匿名访问、推广页实际引用图片仍可访问。然后继续检查带哈希的实际前台 JS/CSS 资源、Dashboard 和公网 Admin API 拒绝策略。Dashboard 验证会以发布 SHA 追加缓存穿透参数，并递归检查入口 HTML 引用及 JS 中声明的所有懒加载 JS/CSS；任一资源 404、状态码或 MIME 类型异常均视为发布失败并回滚。`/assets/` 目录本身不是有效静态资源验收地址。
...
`CONTROLLED_TEST_PAYMENTS_ENABLED` 默认关闭。只有明确批准在目标环境开放测试支付后，才在生产环境文件中设为 `true`；API 与 Worker 必须使用相同值。通过正常 PR、CI 和不可变制品流程发布支持该功能的代码后，再按受审配置重启进程；不要修改运行包内文件或恢复旧的 dummy 支付。

管理员在目标店铺的“支付方式 → 配置测试支付”勾选启用，无需测试客户 ID 或白名单。开启后本店所有客户按平台正常结账规则均可使用。固定代码为 `controlled-test-payment-<公开 Channel ID>`，保留 `controlled-test-payment-checker` 校验服务器开关、店铺与订单状态。修改前记录目标店铺及原配置，修改后通过后台回读并验证不同客户可见。

测试处理器按服务端计算的当前应付金额（包含优惠与已有付款）返回 `Settled`，由 Vendure 正常流程进入 `PaymentSettled`、关闭购物车并触发正常库存与交付。支付元数据保留 `public.testPayment=true`，不发生真实转账；已有报表对测试支付的识别继续有效。上线前须明确目标店铺允许所有客户模拟成功付款，验收使用专用测试商品，不使用真实待发卡库存。测试支付不发起真实退款。

历史 `TestPaymentSettled` / `TestSettled` 记录保持原终态，不自动恢复为真实订单；新的测试订单不再使用这两个状态。验收须区分配置成功、选项可见、实际下单成功，不能用本地 fixture 代替线上证据。
...
备份复用还必须匹配当前备份工具的清单版本。清单算法升级后只创建一次新格式快照；历史快照保留原算法并继续可用于恢复，不修改或删除旧清单。
