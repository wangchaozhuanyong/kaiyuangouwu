...
管理员在目标店铺的“支付方式 → 配置测试支付”填写该店已注册并验证邮箱的测试客户 ID，再勾选启用。固定代码为 `controlled-test-payment-<公开 Channel ID>`，必须保留 `controlled-test-payment-checker`；不得把所有客户加入名单。修改前记录目标店铺、客户 ID 和原支付方式配置，修改后回读后台配置，验证指定客户可见、游客和未指定客户不可见。

测试订单只进入 `TestPaymentSettled`，测试付款只进入 `TestSettled`。它没有真实扣款、发货、库存扣减或收益，不得通过后台转为真实结算/履约，也不支持余额、USDT 或优惠券混用。正常结账仍使用原支付处理器。验收须区分配置成功、选项可见、实际测试订单成功，不能用本地 fixture 代替线上证据。
