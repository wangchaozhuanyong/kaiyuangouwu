...
- 定时换码使用同一条件更新，候选记录发生变化时跳过。访问记录、收信计数与同步状态只更新各自负责的字段；邮件写入在主邮箱行锁内按实际关联记录校准计数，同步不得重新启用已禁用邮箱。
...
- 买家接口固定按 `receivedAt DESC, id DESC` 取最多 5 封，`totalEmails` 是本次展示数量，不提供分页参数；主管理码和后台查询范围保持原状。刷新失败保留上一次列表并显示错误。
- 集成测试支持 `ICLOUD_TEST_PORT` 覆盖默认 HTTP 端口 33551，用于同时运行隔离的 SQL.js 和 MySQL 回归。
