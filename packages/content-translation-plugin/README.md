...
- 英文留空时尝试自动生成；翻译服务限流、超时或不可用时，保留中文与已有英文，将缺失译文记录为 `PENDING`，不因供应商故障回滚商品保存。
- 自动翻译统一先查共享结果缓存，只有未命中的原文才调用翻译服务；同一批和同一进程内并发的相同文字合并请求。已有内容中未变化的英文仍直接复用。
...
- 当前店铺存在 `STALE`、`PENDING`、`TRANSLATING` 或 `FAILED` 内容时，管理后台通知铃铛会显示英文待处理数量。
- 翻译请求最多等待 10 秒；供应商故障后暂停请求 60 秒。后台每分钟最多重试 20 个字段，单轮最多 45 秒，供应商再次失败即结束本轮。复用已有翻译状态表，不新增数据库迁移。
- 补译写入前重新核对源文案、已有英文和人工锁定状态；重启后可继续补译或补发刷新通知，不覆盖管理员后续修改。必填中文和其他业务校验仍然生效。
...
同一次保存同时包含缓存命中与未命中字段时，未命中部分被供应商拒绝不会丢弃已命中的英文；命中字段正常保存，缺译字段才进入等待补译。

跨进程共享已完成的缓存结果；同一进程内正在执行的相同请求合并等待。多个进程同时首次请求同一未缓存原文，仍可能各调用一次，数据库唯一键保证只保留一条结果。持久化缓存会随不同原文数量增长，本次不自动删除已有缓存或业务数据。
...
该测试使用独立 SQL.js 数据库与模拟 Google 403 响应，验证失败行重试、362 行保存、价格库存、两级分类及恢复补译，不连接生产数据库或真实翻译服务。
