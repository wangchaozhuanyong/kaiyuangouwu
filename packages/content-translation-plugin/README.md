...
- 人工填写的英文会被锁定。之后修改中文时，保存会保留该人工英文并将状态标记为 `STALE`，避免覆盖人工内容或阻断日常编辑；运营人员可切换到英文内容完成复核，保存后恢复为 `MANUAL_LOCKED`。
- 当前店铺存在 `STALE` 内容时，管理后台通知铃铛会显示英文待复核数量，并提示运营人员切换到英文内容完成复核。
- 必填英文生成失败时保存失败，不会把只有中文的新增内容发布给英文客户端。
...
历史上在英语语言槽中填写、但实际包含中文字符的原生内容，会由数据库迁移先复制为 `zh_Hans` 源内容。回填时，只有与中文源完全相同且没有人工锁定状态的旧英语值才会被机器翻译替换；已经人工填写且与中文不同的英语仍会保留并锁定。
