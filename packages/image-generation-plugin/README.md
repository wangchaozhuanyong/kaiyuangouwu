...
每个模型都可执行“只读测试”和明确需要确认的“付费生图测试”。只读测试优先读取单模型元数据，再回退到模型列表；不会发起生图。健康结果 24 小时后过期，修改中转站模型 ID 或协议也会立即清空旧状态。
...
1. 运行全部待执行数据库迁移，并确认最新的 `1787817600000-harden-image-generation` 已完成。
