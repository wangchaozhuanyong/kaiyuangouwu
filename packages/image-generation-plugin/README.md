...
插件默认使用手动激活。设置 `IMAGE_PROMPT_SKILL_AUTO_ACTIVATE=true` 后，只有当前进程首次发现的新哈希会自动成为当前版本；旧进程重启不会把历史 bundle 重新激活。历史版本始终保留，可在后台手动回滚。
...
1. 运行全部待执行数据库迁移，并确认最新的 `1787896800000-harden-image-generation-reliability` 已完成。
...
最小付费矩阵固定为：GPT Image 1、1.5、2 各一张 1K；GPT Image 2 一张 16:9 4K；Gemini 一张 1:1 1K 和一张 1:1 4K；OpenAI、Gemini 各一次参考图任务。每张都要核对真实像素、request ID、任务结算、退款、下载和中转站账单。该矩阵只能在明确的预生产地址、测试客户和专用额度下执行，禁止从本地脚本默认连接生产环境。

## 运行健康与报警

`GET /image-generation/health` 同时报告 Worker 心跳、队列积压、24 小时成功率、UNKNOWN 率、失败分类、成本缺失率和 Key 冗余。Worker 超过 90 秒无心跳或最老排队任务超过 2 分钟时返回非 2xx；服务启用但最近 24 小时样本不足 5 次时，近期调用状态为 `INSUFFICIENT_DATA`，不会因小样本阻止发布。

样本达到 5 次后，成功率低于 80% 或 UNKNOWN 率达到 10% 会记录严重告警；成本缺失率达到 20% 以及单 Key 会记录警告。单 Key 警告不会强制关闭服务，但必须明确接受没有自动故障切换的风险。
