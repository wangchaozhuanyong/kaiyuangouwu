...
- 可选 1:1、3:4、4:3、9:16、16:9，固定 1K，每次 1–4 张。
- 免费智能优化描述，用户可继续编辑或一键恢复原文。
- 根据任务自动推荐模型，也允许用户手动选择。
- 历史记录、下载、删除、再次创作、取消尚未执行的单张任务。
...
每个模型都可执行“保存并测试”。测试优先读取单模型元数据，再回退到模型列表；不会发起生图。如果修改了中转站模型 ID 或协议，旧的健康状态会自动清空。元数据请求是否单独收费仍以中转站的计费规则为准。
...
- 管理员可明确选择用原幂等键重试 `UNKNOWN` 输出，或对已成功图片手工退款。
...
1. 运行数据库迁移 `1787792400000-add-image-generation`。
2. 配置上述环境变量，确保 API/worker 共享存储和密钥。
3. 超级管理员配置中转站并执行“测试连接”。
4. 店铺管理员设置模型协议、中转站模型 ID、用途说明、单价和默认模型，执行“保存并测试”后再开启功能。
...
2. 每种协议至少生成一张 1:1 和一张非方形图，核对图片格式、尺寸、中转站 request ID 和实际账单。
