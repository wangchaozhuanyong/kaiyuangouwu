...
本工具处理已结束的旧提示词费用汇总、已落账的提示词逐次调用和单次生图成本事件。历史费用独立存入两张审计表；不会补造旧调用 UUID、逐次记录、响应头编号或完成时间。`PROMPT_ATTEMPT` 必须按真实逐次账本记录审定，父提示词费用由所有逐次调用的已核实金额自动汇总；工具仍不会猜测供应商编号的映射。
...
1. 用 `--manifest <目标清单绝对路径> --output <新快照绝对路径> --inspect` 读取限定频道和目标的最新摘要与前次审定编号。目标类型为 `IMAGE_COST_EVENT`、`LEGACY_PROMPT` 或 `PROMPT_ATTEMPT`；逐次提示词调用使用 `PROMPT_ATTEMPT` 的真实 attempt ID，例如 `{ "version": 1, "targets": [{ "channelId": "1", "recordType": "PROMPT_ATTEMPT", "recordId": "12" }] }`。
...
`PROMPT_ATTEMPT` 只有在父记录为已结束的 v1 逐次账本、调用数量和完成状态完整时才可审定。每次写入会更新该调用的费用，并在同一事务内重新计算父记录汇总；缺少任一调用费用、存在多币种或合计超出字段范围时，父记录保持未知或部分状态，不能伪装成单一已核实金额。
