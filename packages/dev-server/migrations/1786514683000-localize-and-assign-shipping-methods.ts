...
export class LocalizeAndAssignShippingMethods1786514683000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            ...
            FROM "shipping_method" shipping_method
            CROSS JOIN "channel" channel
            WHERE shipping_method."code" IN ('standard-shipping', 'express-shipping')
              AND channel."code" IN ('cn-mainland', 'my-malaysia')
              AND NOT EXISTS (
                  SELECT 1
                  FROM "shipping_method_channels_channel" existing
                  WHERE existing."shippingMethodId" = shipping_method."id"
                    AND existing."channelId" = channel."id"
              )
        `);
