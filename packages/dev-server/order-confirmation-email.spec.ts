...
describe('digital order confirmation email', () => {
    it('normalizes and uses the order-level delivery email for a digital order', () => {
        expect(normalizeDeliveryEmail('  Buyer+Digital@Example.COM ')).toBe(
        ...
        ).toBe('buyer+digital@example.com');
    });

    it('falls back to the customer email for old digital orders without a delivery email', () => {
        expect(orderConfirmationRecipient(true, null, 'account@example.com')).toBe(
            'account@example.com',
        );
    });
    ...
    it('keeps physical order email routing unchanged', () => {
        expect(
            orderConfirmationRecipient(false, 'other@example.com', 'account@example.com'),
