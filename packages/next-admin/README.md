...
已登录且具有当前店铺 `ReadOrder` 权限的管理员，在任意后台页面通过一条 SSE 长连接接收
新订单事件。订单事务提交后，`OrderPlacedEvent` 驱动后端推送，前端收到后立即显示通知并播音。
新订单提醒后 30 分钟仍待处理，推送“您有未处理的订单，请您及时处理。”，之后每 30 分钟重复。
计时器在服务端按待处理订单激活，页面刷新不重置；到期只查询这一张订单，核实未处理后才播报。
“未处理”沿用后台待履约状态（支付已授权、已支付、部分发货、部分交付），并核对未履约数量。
全部商品发货或交付完成、订单取消后停止；部分处理继续提醒，仅打开页面不算处理。
已自动交付的数字商品不会因仍处于支付授权状态而重复提醒。

服务端启动时分页读取一次现有待处理订单以恢复计时器，按下单后的下一个 30 分钟节点继续提醒，
不把历史订单播成新订单。之后没有循环扫描；没有待处理订单时不查询订单。

顶部声音按钮可启用并试听，播放成功后再次点击可静音；静音偏好按管理员保存在当前浏览器，
静音时仍显示订单提醒文字。默认尝试播放，浏览器阻止自动播放时会提示点击启用。
切换店铺或退出登录会关闭旧连接和声音，断线才进行退避重连；凭证或权限失效后停止重试。
服务端每 25 秒写入同一连接的保活注释，不调用订单或会话查询。

后端按实际订单所属 Channel 分发事件。API 和 worker 使用当前部署的同机同用户运行模式，
worker 通过运行目录隔离的私有 Unix socket 发送下单及状态变化事件，无需新增消息服务、配置或数据库表。
实际事件到达或待处理订单的提醒到期时才核对对应订单，投递时再检查会话权限。
服务端内存保留最近 200 条事件供断线续传，新订单按订单 ID 去重，重复提醒按事件 ID 去重；同一订单只保留最近一次超时提醒。
订单处理后移除其超时提醒缓存；新打开页面不会播历史事件。
服务器重启或断线期间超过此缓冲范围时，不保证补发历史新订单事件；待处理订单仍会继续按时提醒。
跨主机、多 API 实例部署需改用共享事件传输，当前实现对应现有单机 API / worker 部署。

语音以页面 `html.lang` 为准：`en` / `en-*` 使用英文，中文使用中文。当前后台入口默认
`zh-CN`，尚无完整后台英文界面切换；店铺内容的默认语言不决定后台语音语言。
四段本地 MP3 随构建发布，无运行时语音服务或系统语音包依赖：

- `public/audio/new-order-zh.mp3`：您有新的订单，请您查看。
- `public/audio/new-order-en.mp3`：You have a new order. Please check it.
- `public/audio/pending-order-zh.mp3`：您有未处理的订单，请您及时处理。
- `public/audio/pending-order-en.mp3`：You have pending orders. Please process them promptly.

音频由 macOS `say` 的 Tingting / Samantha 语音以 175 语速合成，再转换为单声道 24 kHz、
64 kbps MP3。四段合计约 92 KB。同一时刻多笔超时提醒共用正在播放的语音，避免反复打断。关闭页面或设备休眠后无法保证及时播音。

发布时需同时包含 `next-admin`、`next-admin-plugin` 和 Nginx 的 `/admin-order-events` 路由，
该路由在商城与管理域名入口均关闭代理缓冲，并沿用现有管理员鉴权。
