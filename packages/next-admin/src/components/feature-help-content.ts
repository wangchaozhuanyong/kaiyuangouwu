...
export const featureHelpContent = {
    'sales.profit': {
        ...
        requirements: ['具有订单和商品经营数据查看权限', '成本或实际费用缺失时先补齐；无费用必须明确填写 0'],
        example: '例如：实收 100、退款 10、商品成本 40、物流成本 5、手续费 2、拒付 3，净利润为 40。',
        impact: '只读报表；买家支付的运费已计入实收，不重复增加。费用缺失时不生成误导性的净利润。',
    },
    'sales.order-expenses': {
        purpose: '记录实际物流成本、支付手续费和拒付损失，用于订单利润核算，也支持表格批量导入。',
        requirements: [
            '具有订单和商品经营数据修改权限',
            '订单必须属于当前店铺和指定币种',
            '无费用时明确填写 0，缺失时留空',
    ...
    'marketing.attribution': {
        purpose: '按来源、媒介和活动查看访客到付款的转化漏斗、净收入及扣除退款后的投放回报。',
        requirements: ['先选择统计日期范围', '前台访客已同意分析类 Cookie 后才记录归因参数'],
        example: '例如：比较 google / cpc 两个活动的付款订单、净收入、ROAS 和 ROI。',
        impact: '报表只读；订单付款时会冻结 30 天内最后一次非直接来源，后续清理原始访问记录不影响订单归因。',
    },
    'marketing.attribution-cost': {
        purpose: '以追加记录方式登记某个来源、媒介和活动在指定日期的投放费用。',
        requirements: ['具有营销数据修改权限', '币种必须与当前店铺一致', '冲正已有费用时填写负数并说明原因'],
        example: '例如：为 google / cpc / september-sale 在 9 月 21 日登记 500 元广告费。',
        impact: '写入投放成本账本并影响 ROAS 与 ROI；同一幂等键不会重复记账，历史记录不会被覆盖。',
    },
    ...
    'marketing.poster-templates': {
        purpose: '按当前店铺选择五款通用海报，也可复制为本店模板，编辑双语文案、背景与显示开关。',
        requirements: [
            '背景建议 1080×1920，店名、网址和二维码由当前店铺动态生成',
            '按七组固定位置填写中英文，右侧预览均通过后保存',
        ],
        example: '例如：开启两款通用模板，再上传本店背景创建自定义模板，并将其中一款设为默认。',
        impact: '仅影响当前店铺的新海报。开关保留原文案与背景；全部关闭后客户端隐藏生成入口，已保存图片保持原样。',
    },
    ...
    'settings.payment-shipping': {
        ...
        requirements: [
            '只能选择当前环境已提供的支付处理器，新增名称不会开启未提供的支付功能',
            '测试支付需由服务器开放并在本店启用；所有客户均可使用，按应付金额模拟成功并进入正常订单流程',
            '配送试算通过后再向买家开放',
        ],
        example: '例如：启停已配置的支付方式，并配置满 199 元免运费的配送方式。',
    ...
    'settings.data-management': {
        purpose: '查看同意证据、个人数据导出、账户注销、到期清理、恢复区、引用阻断、失败重试和法律保留记录。',
        requirements: [
            '只有超级管理员可以进入',
            '设置法律保留必须填写可追溯原因',
            '解除保留或重试前确认数据已到保留期且不再被业务引用',
            '账户注销重试前处理未完成订单、提现、支付核对、售后和生图任务',
            '同意证据只读保留，不应通过后台覆盖或删除',
        ],
        example: '例如：先处理注销申请显示的业务阻断，再安排重试；退款争议期间则设置法律保留。',
        impact: '注销会撤销访问并匿名化直接身份数据，但依法需要的订单、支付、退款与争议记录继续保留；当前头像不会因时间自动删除。',
    },
