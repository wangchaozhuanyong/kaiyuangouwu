...
export function CommerceModePanel({
    ...
    if (modeQuery.loading && !modeQuery.data) {
        return (
            <section
                className="min-h-[220px] rounded-xl border border-slate-200 bg-white p-5"
                role="status"
                aria-label="正在读取当前店铺经营模式"
            >
                <span className="sr-only">正在读取当前店铺经营模式…</span>
                <div className="h-4 w-40 rounded bg-slate-200" aria-hidden="true" />
                <div className="mt-2 h-3 w-72 max-w-full rounded bg-slate-100" aria-hidden="true" />
