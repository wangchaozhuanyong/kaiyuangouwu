                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            {error && (
                                <div
                                    role="alert"
                                    aria-live="assertive"
                                    className="flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm leading-5 text-rose-700"
                                >
                            ...
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                ...
                                <ul className="mt-2 space-y-1.5 text-xs text-slate-500">
                                    ...
                                    <Requirement met={/[\p{P}\p{S}]/u.test(newPassword)}>
                                        包含符号
                                    </Requirement>
                                </ul>
                            </div>
                            ...
                            <button
                                ...
                                className="flex h-10 w-full items-center justify-center gap-2 rounded-lg text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-wait disabled:opacity-50"
                            >
                                {loggingOut ? (
...
function Requirement({ met, children }: { met: boolean; children: React.ReactNode }) {
    return (
        <li className={`flex items-center gap-2 ${met ? 'text-emerald-700' : ''}`}>
            <CheckCircle2
                className={`h-3.5 w-3.5 ${met ? 'text-emerald-600' : 'text-slate-300'}`}
                aria-hidden="true"
            />
            {children}
        </li>
