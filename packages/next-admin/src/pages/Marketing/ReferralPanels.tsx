        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                ...
                <label className="block text-[11px] font-bold text-slate-600">
                    ...
                    >
                        {SYSTEM_POSTER_TEMPLATES.filter(t => (draft.posterTemplates ?? []).includes(t.id)).map(
                            template => (
...
export function PostersPanel({
    ...
    const toggleSystemTemplate = async (templateId: string, enabled: boolean) => {
        ...
        try {
            ...
            await onChanged(`系统预置模板「${posterLabel(templateId)}」已${enabled ? '开启客户端显示' : '隐藏'}`);
        } catch (error) {
    ...
    const toggleCustomTemplate = async (template: ReferralPosterRecord, enabled: boolean) => {
        ...
        if (!enabled && program.defaultPosterTemplate === template.id) {
            ...
            const otherCustom = program.posterTemplateConfigs.filter(t => t.id !== template.id && t.enabled).map(t => t.id);
        ...
        <div className="space-y-6">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs">
                <div className="border-b border-slate-100 pb-4">
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                        系统内置 5 款全屏移动端海报模板（1080×1920）。您可以通过“在客户端显示”开关自由选择哪些在买家端展示；开启的模板会自动与自定义模板一同在前台展示。
                    </p>
                </div>
                            ...
                            >
                                <div>
                                    ...
                                    >
                                        ...
                                        <div>
                                            <div className="text-sm font-bold drop-shadow-sm">{sys.nameZh}</div>
                                ...
                                <div className="p-3 pt-0 space-y-2">
                                    <label className="flex cursor-pointer items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-xs">
                                        <span className="text-[11px] font-medium text-slate-700">在客户端显示</span>
                                        <input
                                            type="checkbox"
                                            checked={isEnabled}
                                            disabled={isProgramBusy}
                                            onChange={e => void toggleSystemTemplate(sys.id, e.target.checked)}
                                ...
                                <div className="p-3 pt-0 space-y-2">
                                    <label className="flex cursor-pointer items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-xs">
                                        ...
                                        <input
                                            ...
                                            disabled={isProgramBusy}
                                            onChange={e => void toggleCustomTemplate(template, e.target.checked)}
                                            className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                        />
                                    </label>
