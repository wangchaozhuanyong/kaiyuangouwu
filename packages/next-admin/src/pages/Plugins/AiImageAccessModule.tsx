        ...
        <div className="flex h-full flex-col bg-slate-50">
            ...
            <main className="mx-auto w-full max-w-none flex-1 space-y-5 overflow-y-auto p-5 sm:p-8">
                ...
                <section className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs leading-5 text-blue-900">
                    <strong>密钥安全说明：</strong>后端只返回是否已配置及末 4 位。留空 API Key
                    会保留原密钥；输入新值才会轮换。
                </section>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs">
            <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex items-start gap-3">
                    <div
                        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg font-mono text-xs font-bold ${value.scope === 'OPENAI' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}
                    >
                        {value.scope === 'OPENAI' ? 'OA' : 'G'}
                    </div>
                    <div>
                        ...
                        {healthMessage && (
                            <p
                                className={`mt-2 text-[11px] ${health === 'UNHEALTHY' ? 'text-rose-600' : 'text-slate-500'}`}
                            >
                                {healthMessage}
                            </p>
                        )}
                ...
                <button
                    ...
                    disabled={testState.loading || !value.credentialConfigured || dirty}
                    title={dirty ? '请先保存凭据变更' : undefined}
                    className="flex items-center gap-1.5 self-start rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700 disabled:opacity-40"
                >
                    {testState.loading ? (
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <Field label={`API Key ${value.credentialConfigured ? '（留空保留原密钥）' : '*'}`}>
                    <input
                        type="password"
