...
function AppRoutes() {
    ...
    useEffect(() => {
        if (location.pathname === '/login' || extensionsReady) return;
        let cancelled = false;
        void loadInstalledExtensions()
            ...
            .catch(error => {
                if (!cancelled)
                    setExtensionError(error instanceof Error ? error : new Error('扩展页面加载失败'));
            });
        return () => {
            cancelled = true;
        };
    }, [location.pathname, extensionsReady]);

    if (extensionError) throw extensionError;
    if (location.pathname !== '/login' && !extensionsReady) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50 text-sm font-medium text-slate-500">
                正在加载管理后台...
            </div>
        ...
        <Routes>
            ...
            <Route path="/" element={<AuthenticatedShell />}>
                ...
                <Route path="catalog">
                    ...
                    <Route path="collections" element={<Navigate to="/catalog/categories" replace />} />
                    <Route
                        path="option-groups"
                        element={<Navigate to="/catalog/categories?tab=options" replace />}
                    />
                    <Route path="facets" element={<Navigate to="/catalog/categories?tab=facets" replace />} />
                    <Route
                ...
                {getNextAdminExtensionRoutes().map(route => {
                    ...
                    return <Route key={route.id} path={route.path.slice(1)} element={<Component />} />;
                })}
                ...
                <Route path="settings">
                    ...
                    <Route path="stores" element={<Navigate to="/settings/store-profile" replace />} />
                    <Route
                        path="sellers"
                        element={<Navigate to="/settings/store-profile?tab=sellers" replace />}
                    />
                    <Route path="roles" element={<Navigate to="/settings/team?tab=roles" replace />} />
                    <Route path="job-queue" element={<Navigate to="/settings/system-ops" replace />} />
                    <Route
