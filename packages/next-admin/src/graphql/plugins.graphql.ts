...
export type ImageProviderProtocol = 'OPENAI_RESPONSES_IMAGE' | 'OPENAI_IMAGES' | 'OPENAI_COMPATIBLE_CHAT' | 'GEMINI_INTERACTIONS' | 'GEMINI_NATIVE';
...
export interface ImageGenerationAdminResult {
  ...
  imagePromptSkillReleases: Array<{ id: string; createdAt: string; updatedAt: string; bundleVersion: number; sourceHash: string; status: string; activatedAt: string | null }>;
}
...
export interface ContentTranslationAuditResult {
  activeChannel: { id: string; code: string; defaultLanguageCode: string; availableLanguageCodes: string[] };
