...
export function DataTableDateTimeFilter({
    ...
    useEffect(() => {
        ...
        if (result.error === 'Please enter both start and end dates') {
            setError(t`Please enter both start and end dates`);
        } else if (result.error === 'Start date must be before end date') {
            ...
            <div className="flex flex-col md:flex-row gap-2">
                <Select
                    items={Object.fromEntries(
                        DATETIME_OPERATORS.map(op => [op, <HumanReadableOperator key={op} operator={op} />]),
                    )}
                    value={operator}
                    onValueChange={value => {
