...
const localeMessages = import.meta.glob<{ messages: Messages }>('../../i18n/locales/*.po');

export async function loadI18nMessages(locale: string): Promise<Messages> {
    const catalogLocale = locale.replace(/-/g, '_');
    const loader = localeMessages[`../../i18n/locales/${catalogLocale}.po`];
    if (!loader) {
    ...
    const pluginTranslationsForLocale = pluginTranslations.default[catalogLocale] ?? {};
