...
describe('normalizeBreadcrumb', () => {
    ...
    describe('React element handling', () => {
        it('uses the active catalog translation for compiled Lingui messages', () => {
            ...
            expect(
                normalizeBreadcrumb(element, new WeakSet(), descriptor =>
                    descriptor.id === 'new-promotion-id' ? '新建促销活动' : (descriptor.message ?? ''),
                ),
            ).toBe('新建促销活动');
        });
