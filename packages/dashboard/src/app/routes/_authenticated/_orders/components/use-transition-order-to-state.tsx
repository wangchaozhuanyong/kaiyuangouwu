...
export function useTransitionOrderToState(orderId: string | undefined) {
    ...
    const transitionToPreModifyingState = async () => {
        if (data && orderId) {
            ...
            return t`Could not find the order state before modification`;
        }
    };
        ...
        return (
            <Dialog open={selectStateOpen} onOpenChange={setSelectStateOpen}>
                <DialogContent>
                    ...
                    <Select
                        items={Object.fromEntries(props.availableStates.map(state => [state, state]))}
                        value={selectedState}
                        onValueChange={value => {
                            if (value != null) setSelectedState(value);
                        }}
                    >
                        <SelectTrigger>
                            <SelectValue placeholder={t`Select an order state`} />
                        </SelectTrigger>
                        <SelectContent>
