...
export function AssignFacetValuesDialog({
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: () => {
            toast.error(t`Failed to update filter attributes for ${entityIdsLength} items`);
        },
    });

    const handleAssign = () => {
        if (selectedValues.length === 0 && !facetValuesRemoved) {
            toast.error(t`Select at least one filter attribute value or change an existing value`);
            return;
        }
                ...
                <div className="flex-1 overflow-hidden flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <FacetValueSelector
                            ...
                            placeholder={t`Search filter attribute values...`}
                        />
                    </div>
