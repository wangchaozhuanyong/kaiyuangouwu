...
function TaxRateDetailPage() {
    ...
    return <TaxRateEditor taxRateId={params.id} />;
}

export interface TaxRateEditorProps {
    taxRateId: string;
...
export function TaxRateEditor({
    ...
    onRequestClose,
    onSaved,
}: Readonly<TaxRateEditorProps>) {
    const navigate = useNavigate();
    const queryClient = useQueryClient();
    const createAnotherRef = useRef(false);
    const creatingNewEntity = taxRateId === NEW_ENTITY_PATH;
    const { t } = useLingui();
            ...
            <PageActionBar>
                {presentation === 'sheet' && (
                    <Button
                        type="button"
                        variant="outline"
                        render={<Link to={`/tax-rates/${taxRateId}`} preload={false} />}
                    >
                ...
                <ActionBarItem
                    itemId="save-button"
                    requiresPermission={[creatingNewEntity ? 'CreateTaxRate' : 'UpdateTaxRate']}
                >
                    <Button
                        type="submit"
                        disabled={!form.formState.isDirty || !form.formState.isValid || isPending}
