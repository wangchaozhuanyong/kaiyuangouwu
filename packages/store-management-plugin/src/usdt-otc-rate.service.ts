...
const BINANCE_P2P_AD_LIST_URL =
    'https://www.binance.com/bapi/c2c/v1/public/c2c/agent/ad-list?fiat=CNY&asset=USDT&tradeType=SELL&limit=20';
const OKX_P2P_AD_LIST_URL =
    'https://www.okx.com/v3/c2c/tradingOrders/books?' +
    'quoteCurrency=CNY&baseCurrency=USDT&side=buy&paymentMethod=all&userType=merchant&' +
...
interface OkxP2pAdListResponse {
    code?: number;
    data?: { buy?: OkxP2pAdvertisement[] } | null;
}
...
export class UsdtOtcRateService {
    async fetchCnyRate(): Promise<UsdtOtcRateSnapshot> {
        ...
        return {
            cnyPerUsdtRate,
            source: `${sourceNames} 认证商家收购 USDT 等权中位价（${sampledAdvertisementCount} 条）`,
    ...
    private async fetchOkxRate(): Promise<UsdtOtcSourceSnapshot> {
        ...
        const prices = selectReliableOkxMerchantPrices(payload.data?.buy ?? []);
