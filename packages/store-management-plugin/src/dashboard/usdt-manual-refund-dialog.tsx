...
export function UsdtManualRefundDialog({
    ...
    const mutation = useMutation({
        ...
        onSuccess: async result => {
            ...
            setOpen(false);
            resetForm(remainingAmount, setFiatAmount, setUsdtAmount, setTransactionId, setReason);
            await onRecorded();
        },
            ...
            <Dialog open={open} onOpenChange={nextOpen => !mutation.isPending && setOpen(nextOpen)}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            仅在链上退款已经成功后登记。系统会核验交易已固化，并在 Vendure
                            中建立已结算退款；保存后将立即扣减收款统计。
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-2">
                        ...
                        <p className="mb-0 text-xs leading-5 text-muted-foreground">
                            注意：系统核验交易哈希已成功固化，但仍需操作人核对收款地址和实际 USDT
                            数量。退款登记不可作为发起链上转账的功能。
                        </p>
                    </div>
                    <DialogFooter>
                        <Button
