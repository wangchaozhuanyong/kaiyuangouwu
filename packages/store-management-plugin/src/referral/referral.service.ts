...
export class ReferralService implements OnApplicationBootstrap {
    ...
    private async normalizePosterTemplateInput(ctx: RequestContext, input: SaveReferralPosterTemplateInput) {
        ...
        return {
            ...
            qrEyebrowZh: clippedText(input.qrEyebrowZh ?? '扫码访问 AwanMesh 模钥', 100),
            qrEyebrowEn: clippedText(input.qrEyebrowEn ?? 'Scan AwanMesh', 100),
            ...
            ctaTextZh: clippedText(input.ctaTextZh ?? '长按识别二维码，立即进入 AwanMesh 模钥', 140),
            ctaTextEn: clippedText(input.ctaTextEn ?? 'Press and hold to enter AwanMesh', 140),
