/* eslint-disable no-bitwise -- TOTP/Base32 encoding and RFC dynamic truncation are bit-oriented. */

const BASE32_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567';
const DEFAULT_DIGITS = 6;
const DEFAULT_PERIOD_SECONDS = 30;
import { InvalidTwoFactorSecretError, normalizeBase32Secret } from '../two-factor-secret';

export { InvalidTwoFactorSecretError, normalizeBase32Secret } from '../two-factor-secret';
...
export function getTotpSecondsRemaining(timeMs = Date.now(), periodSeconds = DEFAULT_PERIOD_SECONDS): number {
    const elapsed = Math.floor(timeMs / 1000) % periodSeconds;
