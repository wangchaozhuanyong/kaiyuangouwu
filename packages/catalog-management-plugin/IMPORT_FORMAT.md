...
| 商品类型 | 新建商品必填“虚拟货品”或“实物”。兼容“虚拟商品”“实物商品”及 `digital` / `physical`。已有 SKU 的维护文件可不提供此列，系统会保留原类型；不根据名称、库存或门店默认值猜测。                                 |
| 一级分类 | 新建商品必填主分类；兼容旧列名“分类”“商品分类”。已有 SKU 的维护文件可以不提供分类列，以保留当前分类。                                                                                                    |
| 二级分类 | 可空。填写时必须同时填写一级分类。空白表示只属于一级分类，更新时也移除原导入二级分类，无需启用通用空白清除模式。                                                                                         |
| SKU      | 维护已有商品时保留稳定 SKU，以免因名称或分类改变而失去唯一匹配依据。                                                                                                                                     |

维护已有 SKU 时，只有文件实际提供且预览显示发生变化的字段才会写入。仅更新描述可使用“导入商店、SKU、商品描述”三列；不需要复制名称、商品类型、分类、价格或库存。已有商品改到不存在的分类时会被阻止，必须先在分类管理中建立并复核层级；如果一个名称已经作为其他一级分类的二级分类存在，也不能通过导入把它提升成一级分类。

分类名不能包含 `>`，层级必须分列填写。同名二级分类按父分类区分，空白不会新建“默认”“无”或空名称的二级分类。

导出表保留“导入商店”（导出请求的当前渠道标识）、“商品类型”“一级分类”“二级分类”。导出优先使用真实集合父子关系；导入分类标记仅在与集合层级一致或集合层级缺失时回退使用，禁止用排序后的第一个分类名称猜一级分类。商品类型写入现有 Product.fulfillmentType，并由既有履约事件处理器同步 SKU。仅虚拟或仅实物门店遇到不兼容类型会报错，需要先明确调整门店经营模式。
